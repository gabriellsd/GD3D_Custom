...
function detectarExtensaoImagemFbx( content ) {
	...
	if ( typeof content === 'string' ) {
		...
		try {
			...
			const amostra = atob( content.replace( /\s/g, '' ).slice( 0, 32 ) );
			bytes = new Uint8Array( amostra.length );
			for ( let i = 0; i < amostra.length; i ++ ) bytes[ i ] = amostra.charCodeAt( i );
		...
		bytes = new Uint8Array( content, 0, Math.min( 16, content.byteLength ) );

	} else if ( content?.buffer instanceof ArrayBuffer ) {

		bytes = new Uint8Array( content.buffer, content.byteOffset, Math.min( 16, content.byteLength ) );
...
class FBXTreeParser {
	...
	parseImage( videoNode ) {
		...
		const extensoesValidas = new Set( [ 'bmp', 'jpg', 'jpeg', 'png', 'tif', 'tga' ] );

		if ( ! extensoesValidas.has( extension ) ) {

			const detectada = detectarExtensaoImagemFbx( content );
			if ( detectada ) extension = detectada;
		...
		switch ( extension ) {
			...
			default:
				...
				console.warn( 'FBXLoader: Image type "' + ( extension || fileName ) + '" is not supported.' );
				return;
